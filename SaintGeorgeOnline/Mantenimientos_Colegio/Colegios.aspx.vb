...
Partial Class Mantenimientos_Colegio_Colegios
...
#Region "Eventos"
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            Me.Master.MostrarTitulo("Colegios")
            ...
            Controles.CaracteresEspeciales.agregarGrupo(ftbPaginaWeb, 1)
...
#Region "Metodos"
    ...
    Private Sub Exportar()
        ...
        dt = Datos.agregarColumna(dt, "PaginaWeb", "String")
        ...
        For Each dr As DataRow In ds_Lista.Tables(0).Rows
            ...
            auxDR.Item("PaginaWeb") = dr.Item("PaginaWeb").ToString
            auxDR.Item("Estado") = dr.Item("Estado").ToString
            dt.Rows.Add(auxDR)
            cont += 1
        Next

        Dim str_Titulo As String = "Colegios"

        If rbExportar.SelectedValue = 0 Then 'WORD

            Dim reporte_html As String = ""
            Dim Arreglo_Datos As String()
            Arreglo_Datos = Exportacion.ExportarReporte_Html(dt, str_Titulo)

            Dim arrByteData() As Byte = Archivos.StringToArrByte(Arreglo_Datos(0))
            Archivos.Descargar.Word(arrByteData, "Reporte.doc")

        ElseIf rbExportar.SelectedValue = 1 Then 'EXCEL

            NombreArchivo = Exportacion.ExportarReporte(dt, str_Titulo)
            rutamadre = Server.MapPath(".").Replace("\Mantenimientos_Colegio", "\Reportes\")
            downloadBytes = File.ReadAllBytes(rutamadre & NombreArchivo & ".xls")

            Archivos.Descargar.Excel(downloadBytes, "Reporte.xls")

        ElseIf rbExportar.SelectedValue = 2 Then 'PDF

            Dim m As System.IO.MemoryStream = New System.IO.MemoryStream
            m = Exportacion.ExportarReporte_Pdf(dt, str_Titulo)

            Archivos.Descargar.PDF(m, "Reporte.pdf")
    ...
    Private Function validar(ByRef str_Mensaje As String) As Boolean
        ...
        'If tbPaginaWeb.Text.Trim.Length = 0 Then
        '    str_alertas = Alertas.ObtenerAlerta(str_alertas, 1, "Pagina Web")
        '    result = False
        'End If
        If Validacion.ValidarCamposIngreso(tbPaginaWeb) = False Then
            str_alertas = Alertas.ObtenerAlerta(str_alertas, 2, "Pagina Web")
            result = False
        End If
    ...
    Private Function ObtenerResultadoBusqueda(ByVal int_Modo As Integer) As DataSet
        ...
        If int_Modo = 1 Then 'LLAMAR A LA BASE DE DATOS
            ...
            Dim obj_BL_Colegio As New bl_Colegios
            ds_Lista = obj_BL_Colegio.FUN_LIS_Colegios(str_Descripcion, int_Estado, int_CodigoUsuario, int_CodigoTipoUsuario, cod_Modulo, cod_Opcion)
    ...
    Private Sub obtener(ByVal int_Codigo As Integer)
        ...
        Dim ds_Lista As DataSet = obj_BL_Colegio.FUN_GET_Colegios(int_Codigo, int_CodigoUsuario, int_CodigoTipoUsuario, cod_Modulo, cod_Opcion)
        ...
        tbPaginaWeb.Text = ds_Lista.Tables(0).Rows(0).Item("PaginaWeb").ToString
    ...
    Protected Sub cambiarEstado(ByVal int_Codigo As Integer, ByVal str_accion As String)
        ...
        If str_accion = "Eliminar" Then
            usp_valor = obj_BL_Colegio.FUN_DEL_Colegios(int_Codigo, usp_mensaje, int_CodigoUsuario, int_CodigoTipoUsuario, cod_Modulo, cod_Opcion)
        End If
    ...
    Private Sub Grabar()
        ...
        Dim obj_BE_Colegios As New be_Colegios
        Dim obj_BL_Colegio As New bl_Colegios
        ...
        obj_BE_Colegios.Nombre = tbDescripcion.Text.Trim
        obj_BE_Colegios.PaginaWeb = tbPaginaWeb.Text.Trim

        If BoolGrabar = 0 Then
            usp_valor = obj_BL_Colegio.FUN_INS_Colegios(obj_BE_Colegios, usp_mensaje, int_CodigoUsuario, int_CodigoTipoUsuario, cod_Modulo, cod_Opcion)
        Else
            obj_BE_Colegios.CodigoColegio = CInt(BoolGrabar)
            usp_valor = obj_BL_Colegio.FUN_UPD_Colegios(obj_BE_Colegios, usp_mensaje, int_CodigoUsuario, int_CodigoTipoUsuario, cod_Modulo, cod_Opcion)
        End If
