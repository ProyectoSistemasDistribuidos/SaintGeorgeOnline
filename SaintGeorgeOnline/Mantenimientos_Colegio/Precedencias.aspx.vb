...
''' <summary>
''' Modulo de Mantenimiento de Precedencias
''' </summary>
''' <remarks>
''' Código del Modulo:   
''' Código de la Opción: 
...
Partial Class Mantenimientos_Colegio_Precedencias
...
#Region "Eventos"
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            Me.Master.MostrarTitulo("Precedencias")
            If Not Page.IsPostBack Then
                SetearAccionesAcceso()
                ViewState("SortExpression") = "CodigoGradoALlevar"
    ...
    Protected Sub btnCerrarPanelPrecedencia_Click()

        miTab1.Enabled = True
        TabContainer1.ActiveTabIndex = 0
        ddlBuscarGradoALlevar.Focus()
        hd_Codigo.Value = 0
...
#Region "Metodos"
    ...
    ''' Fecha de modificación: 31/03/2011
    ''' </remarks>
    ...
    Private Sub limpiarFiltros()
        ...
        ddlBuscarGradoALlevar.SelectedValue = 0
        ddlBuscarGradoALLevar.Focus()
    ...
    Private Sub cargarComboGrados()
        ...
        Controles.llenarCombo(ddlBuscarGradoALlevar, ds_Lista, "Codigo", "Descripcion", True, False)
        Controles.llenarCombo(ddlGradoALlevar, ds_Lista, "Codigo", "Descripcion", False, True)
        Controles.llenarCombo(ddlGradoRequisito, ds_Lista, "Codigo", "Descripcion", False, True)
    ...
    Private Sub Exportar()
        ...
        str_TituloCabeceraArchivo = "Precedencias"
        ...
        dt = Datos.agregarColumna(dt, "NivelALLevar", "String")
        dt = Datos.agregarColumna(dt, "GradoALlevar", "String")
        dt = Datos.agregarColumna(dt, "NivelRequisito", "String")
        dt = Datos.agregarColumna(dt, "GradoRequisito", "String")
        ...
        For Each dr As DataRow In ds_Lista.Tables(0).Rows
            auxDR = dt.NewRow
            auxDR.Item("N°") = cont
            auxDR.Item("NivelALLevar") = dr.Item("NivelALLevar").ToString
            auxDR.Item("GradoALlevar") = dr.Item("GradoALlevar").ToString
            auxDR.Item("NivelRequisito") = dr.Item("NivelRequisito").ToString
            auxDR.Item("GradoRequisito") = dr.Item("GradoRequisito").ToString
            dt.Rows.Add(auxDR)
            cont += 1
        Next
    ...
    Private Function validar(ByRef str_Mensaje As String) As Boolean
        ...
        If ddlGradoALLevar.SelectedValue = 0 Then
            str_alertas = Alertas.ObtenerAlerta(str_alertas, 3, "Grado a LLevar")
            result = False
        End If
    ...
    Private Sub limpiarCampos()
        ...
        hd_Codigo.Value = 0
        ddlGradoALlevar.SelectedValue = 0
        ddlGradoRequisito.SelectedValue = 0

    End Sub

    ''' <summary>
    ''' Lista los datos de Precedencias   
    ''' </summary>
    ''' <remarks>
    ''' Creador:               Fanny Salinas 
    ''' Fecha de Creación:     13/01/2011
    ...
    Private Function ObtenerResultadoBusqueda(ByVal int_Modo As Integer) As DataSet
        Dim int_GradoALlevar As Integer = ddlBuscarGradoALlevar.SelectedValue
        ...
        If int_Modo = 1 Then 'LLAMAR A LA BASE DE DATOS
            ...
            Dim obj_BL_Precedencias As New bl_Precedencias
            ds_Lista = obj_BL_Precedencias.FUN_LIS_Precedencias(int_GradoALlevar, int_Estado, int_CodigoUsuario, int_CodigoTipoUsuario, cod_Modulo, cod_Opcion)
    ...
    ''' </summary>
    ''' <param name="int_Codigo">Código de Precedencias</param>
    ''' <remarks>
    ''' Creador:               Fanny Salinas 
    ''' Fecha de Creación:     13/01/2011
    ''' Modificado por:        _____________
    ...
    Private Sub obtener(ByVal int_Codigo As Integer)
        ...
        Dim ds_Lista As DataSet = obj_BL_Precedencias.FUN_GET_Precedencias(int_Codigo, int_CodigoUsuario, int_CodigoTipoUsuario, cod_Modulo, cod_Opcion)

        hd_Codigo.Value = CInt(ds_Lista.Tables(0).Rows(0).Item("Codigo").ToString)
        ddlGradoALlevar.SelectedValue = ds_Lista.Tables(0).Rows(0).Item("CodigoGradoALlevar").ToString
        ddlGradoRequisito.SelectedValue = ds_Lista.Tables(0).Rows(0).Item("CodigoGradoRequisito").ToString
    ...
    Protected Sub cambiarEstado(ByVal int_Codigo As Integer, ByVal str_accion As String)
        ...
        If str_accion = "Eliminar" Then
            usp_valor = obj_BL_Precedencias.FUN_DEL_Precedencias(int_Codigo, usp_mensaje, int_CodigoUsuario, int_CodigoTipoUsuario, cod_Modulo, cod_Opcion)
        End If
    ...
    Private Sub Grabar()
        ...
        Dim obj_BE_Precedencias As New be_Precedencias
        Dim obj_BL_Precedencias As New bl_Precedencias
        ...
        obj_BE_Precedencias.CodigoGradoALlevar = ddlGradoALlevar.SelectedValue
        obj_BE_Precedencias.CodigoGradoRequisito = ddlGradoRequisito.SelectedValue

        If BoolGrabar = 0 Then
            usp_valor = obj_BL_Precedencias.FUN_INS_Precedencias(obj_BE_Precedencias, usp_mensaje, int_CodigoUsuario, int_CodigoTipoUsuario, cod_Modulo, cod_Opcion)
        Else
            obj_BE_Precedencias.CodigoPrecedencia = CInt(BoolGrabar)
            usp_valor = obj_BL_Precedencias.FUN_UPD_Precedencias(obj_BE_Precedencias, usp_mensaje, int_CodigoUsuario, int_CodigoTipoUsuario, cod_Modulo, cod_Opcion)
        End If
...
#Region "Metodos del Gridview"
    ...
    Private Sub ImagenSorting(ByVal nombreBoton As String)
        ...
        Dim _btnSorting_d1 As ImageButton = CType(GridView1.HeaderRow.FindControl("btnSorting_NivelALLevar"), ImageButton)
        Dim _btnSorting_d2 As ImageButton = CType(GridView1.HeaderRow.FindControl("btnSorting_CodigoGradoALlevar"), ImageButton)
        Dim _btnSorting_d3 As ImageButton = CType(GridView1.HeaderRow.FindControl("btnSorting_NivelRequisito"), ImageButton)
        Dim _btnSorting_d4 As ImageButton = CType(GridView1.HeaderRow.FindControl("btnSorting_CodigoGradoRequisito"), ImageButton)
