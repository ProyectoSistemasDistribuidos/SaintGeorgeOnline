...
Partial Class Modulo_BancoLibros_DevolucionLibros
...
#Region "Eventos "
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            ...
            Me.Master.MostrarTitulo("Registro de Devolución de Libros")
...
#Region "Metodos"
    ...
    Private Sub listar()
        ...
        If ds_Lista.Tables(0).Rows.Count > 0 Then
            ...
            ViewState("ListaLibrosCadena") = ds_Lista.Tables(1)
            ViewState("ListaLibrosMarcados") = ds_Lista.Tables(2)

            GridView1.DataSource = ds_Lista.Tables(0)
            GridView1.DataBind()

            dl_NombreLibros.DataSource = ds_Lista.Tables(1)
    ...
    Private Function ObtenerResultadoBusqueda(ByVal int_Modo As Integer) As DataSet
        ...
        If int_Modo = 1 Then 'LLAMAR A LA BASE DE DATOS
            ...
            ds_Lista = obj_BL_Devoluciones.FUN_LIS_PrestamoPorLibroGradoPeriodo(int_Periodo, int_Grado, int_Aula, int_Idioma, int_CodigoUsuario, int_CodigoTipoUsuario, cod_Modulo, cod_Opcion)
    ...
    ''' Fecha de modificación: _____________ 
    ''' </remarks>
    Private Function ObtenerTablaPrestamo() As DataTable
        Dim dt As DataTable
        dt = New DataTable("ListaLibrosMarcados")
        dt = Datos.agregarColumna(dt, "CodigoPrestamo", "Integer")
        dt = Datos.agregarColumna(dt, "CodigoDetallePrestamo", "Integer")
    ...
    ''' Fecha de modificación: _____________ 
    ''' </remarks>
    Private Function setearDetalleTabla(ByVal dt_Libros As DataTable, ByVal dt_LibrosAlumno As DataView, ByVal str_CodigoAlumno As String, ByVal chkLibros As WebControls.CheckBoxList, ByRef bool_TienePrestamos As Boolean) As Boolean
        ...
        Dim bool_Prestamo As Boolean = False

        For Each item As ListItem In chkLibros.Items
            item.Attributes.Add("style", "visibility:hidden;")
            chkLibros.Items(contObj).Enabled = False
        Next
            ...
            If dr.Item("CodigoAlumno") = str_CodigoAlumno Then
                While contObj <= chkLibros.Items.Count - 1
                    If chkLibros.Items(contObj).Value = dr.Item("CodigoLibro") Then
                        ...
                        chkLibros.Items(contObj).Enabled = True
                        chkLibros.Items(contObj).Attributes.Remove("style")
                        bool_Prestamo = True

                        If Convert.ToInt32(dr.Item("EstadoPrestamo")) = 0 Then ' Estado Prestamo : Pendiente
                            chkLibros.Items(contObj).Selected = False
                        ElseIf Convert.ToInt32(dr.Item("EstadoPrestamo")) = 1 Then ' Estado Prestamo : Devuelto
        ...
        bool_TienePrestamos = bool_Prestamo

        If int_cantLibros = dt_LibrosAlumno.Count And int_cantLibros > 0 Then
            Return True
        Else
            Return False
        End If
...
#Region "Metodos GridView"
    ...
    Private Sub Grabar(ByVal chkLibrosActualizar As CheckBoxList, ByVal str_CodigoAlumno As String, ByVal str_NombreAlumno As String, ByVal int_CodigoPrestamo As Integer, ByVal gvIndex As Integer, ByVal miArray As Array)
        ...
        Dim usp_valorDetalle As Integer = 0
        Dim usp_valorEliminados As Integer = 0
        ...
        While contObj <= chkLibrosActualizar.Items.Count - 1
            ...
            If chkLibrosActualizar.Items(contObj).Attributes("style") <> "visibility:hidden;" _
                Or chkLibrosActualizar.Items(contObj).Attributes("style") Is Nothing Then
                ...
                If usp_CodigoPrestamoDetalle > 0 Then ' Si existe el detalle

                    If miArray(contObj) = "True" Then ' Si está marcado
                        If usp_EstadoPrestamo = 1 Then ' Si el prestamo ya fue devuelto (Estado 1), no se actualiza ningún valor
                        ...
                        Else ' Si el prestamo esta pendiente (Estado 0), se actualiza a estado Prestamo : 1 (Devuelto)
                            int_Accion = 1 ' Devolución de prestamo, estado final : devuelto
                    ...
                    Else ' si está desmarcado
                        If usp_EstadoPrestamo = 1 Then ' Si el prestamo ya fue devuelto (Estado 1), se actualiza a estado Prestamo : 0 (Pendiente)
                            int_Accion = 0 ' Cancelación de devolución de prestamo, estado final : pendiente
                            bool_Actualizar = True
                        ...
                        Else ' Y el estado del prestamo es 0, no se actualiza ningún valor
                            bool_Actualizar = False
                        End If
                    End If

                    If bool_Actualizar Then
                        usp_valorEliminados = obj_BL_Devoluciones.FUN_UPD_PrestamoDetalle(usp_CodigoPrestamoDetalle, int_Accion, usp_mensaje, int_CodigoUsuario, int_CodigoTipoUsuario, cod_Modulo, cod_Opcion)
                    End If
    ...
    Private Function mensajeRegistro(ByVal dt_Registro As DataTable, ByVal str_NombreAlumno As String) As String
        ...
        Dim int_Registrados, int_Eliminados As Integer

        ' Registrado : 1 - Eliminado : 0
        For Each dr As DataRow In dt_Registro.Rows
            If dr.Item("Tipo") = 1 Then
                int_Registrados += 1
                str_MsjRegistrados.Append("<li style=""width: 300px;"">" & _
                                                "<div style=""width: 300px; border: solid 0px red; float: left; text-align:left;"">" & formatearCadenasHTML(dr.Item("TituloLibro")) & "</div></li>")
        ...
        str_MensajeFinal.Append("<span>Devoluciones del alumno(a) : </span><br><span style=""font-weight: bold;"">" & str_NombreAlumno & "</span><br />")
        str_MensajeFinal.Append("<br /><div style=""font-weight: bold;"">" & _
                                "<div style=""width: 300px; border: solid 0px #000000; float: left; text-align:left;"">Título</div><br />")
        ...
        If int_Registrados > 0 Then
            ...
            str_MensajeFinal.Append("<div style=""font-weight: bold;"">" & _
                                    "<div style=""width: 260px; border-top: solid 1px #000000; float: left; text-align:left;"">Total de prestamos devueltos :</div>" & _
                                    "<div style=""width: 40px; border-top: solid 1px #000000; float: left; text-align:right;"">" & int_Registrados.ToString & "</div></div><br />")
        ...
        If int_Eliminados > 0 Then
            ...
            str_MensajeFinal.Append("</ul>")
            str_MensajeFinal.Append("<div style=""font-weight: bold;"">" & _
                                    "<div style=""width: 260px; border-top: solid 1px #000000; float: left; text-align:left;"">Total de devoluciones canceladas :</div>" & _
                                    "<div style=""width: 40px; border-top: solid 1px #000000; float: left; text-align:right;"">" & int_Eliminados.ToString & "</div></div><br />")
        ...
        str_MensajeFinal.Append("<span>Devoluciones : </span>")
        str_MensajeFinal.Append("<span style=""font-weight: bold; color: black;"">Registrados</span> - ")
        str_MensajeFinal.Append("<span style=""font-weight: bold; color: red;"">Canceladas</span>")
...
#Region "Eventos del Gridview"
    ...
    Protected Sub GridView1_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs)
        ...
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            chkAll.Checked = setearDetalleTabla(dt_Libros, dv, str_CodigoAlumno, chkLibros, bool_TienePrestamos)

            If bool_TienePrestamos = False Then
                chkLibros.Attributes.Add("style", "visibility:hidden;")
                btnEditar.Attributes.Add("style", "visibility:hidden;")
            End If
